...
#     def __init__(self, data_root="./WildDeepfake/deepfake_in_the_wild", output_root="./dataset"):
...
#                     extract_info[split_name].append(str(tar_path))
...
#         data_root="./WildDeepfake",  # tar.gz 文件所在的根目录
#         output_root="./dataset"       # 输出数据集索引的目录
#     )
...
class WildDeepfakePreparator:
    ...
    def is_already_extracted(self, tar_path):
        """检查某个 tar.gz 是否已经解压过"""
        # 解压后会生成以 video_id 命名的文件夹
        # tar.gz 文件名就是 video_id，如 1.tar.gz -> 1/
        video_id = tar_path.stem  # "1.tar" -> "1" (注意 .tar.gz 的 stem)
        if video_id.endswith('.tar'):
            video_id = video_id[:-4]  # 去掉 .tar
